...
GitHub repository for analysis code: [https://github.com/sbresnahan/Winter-Honeybee-Transcriptome](https://github.com/sbresnahan/Winter-Honeybee-Transcriptome)

```{r setup, message=F}
# Set the code chunk options for knitr
...
grep -v '^#' Amel_HAv3.1.gtf | awk '$3=="exon";$3=="CDS" { print $0 }' | \
   sed 's/.*GeneID://' | awk -F'"' '{print $1}' > transcript_GeneIDs.txt

grep -v '^#' Amel_HAv3.1.gtf | awk '$3=="exon";$3=="CDS" { print $0 }' | \
   sed 's/.*transcript_id \"//' | awk -F'"' '{print $1}' > transcript_IDs.txt

paste -d'\t' transcript_IDs.txt transcript_GeneIDs.txt | \
   awk -v OFS="," '{print "rna-"$1, $2}' | sort | uniq > tx2gene.csv
```

```{r, eval=F}
tx2gene <- read.csv("data/index/tx2gene.csv",header=F)
names(tx2gene) <- c("tx_id","gene_id")
write.csv(tx2gene,"data/index/tx2gene.csv")
```

### tximport kallisto abundance.tsv files

```{r, eval=F}
tx2gene <- read.csv("data/index/tx2gene.csv",header=T)
...
write.csv(datExpr,"Manuscript/tabs4.csv")
## Clear all
rm(list=ls())
```
...
datExpr$X <- NULL
datExpr <- datExpr[,as.character(metadata$Accession)]
```
...
```{r}
tabs3 <- data.frame(Estimate=colSums(datExpr))
tabs3 <- data.frame(Accession=row.names(tabs3),Estimate=tabs3$Estimate)
tabs3 <- plyr::join(tabs3,metadata,by="Accession")
tabs3 <- tabs3[,c(1,4,3,6,2)]
tabs3 <- tabs3[order(tabs3$Tissue,tabs3$Label),]
tabs3$Tissue <- revalue(tabs3$Tissue, c("A"="Fat body", "T"="Flight muscle"))
mean(tabs3$Estimate)
tabs3 %>% 
  kbl(label="Table S3", caption = "RNAseq library metadata and pseudoalignment rates on the Amel_HAv3.1 kallisto reference transcriptome",
      row.names=F) %>% kable_classic(full_width = F)
write.csv(tabs3,"Manuscript/tabs3.csv")
```
...
abdomen_kmeans.plot <- mod.fviz_cluster(dds.A.transform,
                                        ...
                                        showLegend=F,K=2)
ggsave(file="Manuscript/fig2a.pdf", plot=abdomen_kmeans.plot, 
       width=6, height=5)
...
thorax_kmeans.plot <- mod.fviz_cluster(dds.T.transform,
                                       ...
                                       showLegend=T,K=2)
ggsave(file="Manuscript/fig2b.pdf", plot=thorax_kmeans.plot, 
       width=6, height=5)
...
abdomen_kmeans.plot <- mod.fviz_cluster(FvN.A.union.dds.transform,
                                        ...
                                        lim=50)
abdomen_kmeans.plot
ggsave(file="Manuscript/figs5a.pdf", plot=abdomen_kmeans.plot, 
       width=6, height=5)
...
thorax_kmeans.plot <- mod.fviz_cluster(FvN.T.union.dds.transform,
                                       ...
                                       lim=50)
thorax_kmeans.plot
ggsave(file="Manuscript/figs5b.pdf", plot=thorax_kmeans.plot, 
       width=6, height=5)
```
